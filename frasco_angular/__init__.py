        ...
        if (disable is None and not self.app.debug) or disable is False:
            ...
            for path, dirnames, filenames in os.walk(os.path.join(self.options["static_dir"], self.options['app_dir'])):
                for filename in filenames:
                    ...
                    if matcher.match(relname):
                        with codecs.open(os.path.join(path, filename), 'r', 'utf-8') as f:
                            module += "  $templateCache.put('%s?%s', %s);\n" % (relname, version, json.dumps(htmlmin.minify(f.read())))
